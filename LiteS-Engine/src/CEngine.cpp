...
void CEngine::runEngine() {
  ...
  extraAlgorithm =
      new std::thread(&CComponent::extraAlgorithm, this->m_Component);
  // initThread.detach();
  ...
  while (!glfwWindowShouldClose(this->m_Window)) {
...

    {
      ...
      // storing our window open/close state
...
void CEngine::handleInput(GLFWwindow* window) {
  ...
  if (isClicked(window, GLFW_KEY_F10)) this->m_Component->stepExtraAlgorithm();
  if (isClicked(window, GLFW_KEY_EQUAL)) glPointSize(pointSize += 1);
...
void mouse_callback(GLFWwindow* window, double xpos, double ypos) {
  if (CEngine::m_mouseClicked) {
    ...
    if (glfwGetKey(window, GLFW_KEY_LEFT_CONTROL) == GLFW_PRESS) {
      if (yoffset > 0)
        CEngine::m_Scene->m_Camera->ProcessPositionMovement(
            DOWN, CEngine::m_deltaTime);
      if (yoffset < 0)
        CEngine::m_Scene->m_Camera->ProcessPositionMovement(
            UP, CEngine::m_deltaTime);
      if (xoffset > 0)
        CEngine::m_Scene->m_Camera->ProcessPositionMovement(
            LEFT, CEngine::m_deltaTime);
      if (xoffset < 0)
        CEngine::m_Scene->m_Camera->ProcessPositionMovement(
            RIGHT, CEngine::m_deltaTime);
    } else {
...
bool CEngine::__readProperties(string configFile) {
  ...
  try {
    ...
    for (rapidjson::SizeType i = 0; i < renderPasses.Size(); i++) {
      ...
      TargetCount), GL_TEXTURE_2D, CEngine::m_Scene->m_Texture.at(OutTexture),
      0);
      ...
      glRenderbufferStorage(
          GL_RENDERBUFFER, GL_DEPTH24_STENCIL8, pass->m_Width,
          pass->m_Height);  // use a single renderbuffer object for both a depth
                            // AND stencil buffer.
      glFramebufferRenderbuffer(GL_FRAMEBUFFER, GL_DEPTH_STENCIL_ATTACHMENT,
                                GL_RENDERBUFFER,
                                DepthRenderBuffer);  // now actually attach it
    ...
    if (!arguments.IsNull()) {
      for (rapidjson::Value::ConstMemberIterator itr = arguments.MemberBegin();
        ...
        if (itr->value.IsString())
          CEngine::m_Arguments.insert(
              {itr->name.GetString(), itr->value.GetString()});
        else if (itr->value.IsNumber())
          CEngine::m_Arguments.insert(
              {itr->name.GetString(),
               LiteUtil::numberToString<double>(itr->value.GetDouble())});
        else {
  ...
  // node->FirstChildElement("FORMAT")->GetText(); 			if
  ...
  //		glTexImage2D(GL_TEXTURE_2D, 0, FormatInt, RenderWidth,
  // RenderHeight, 0, InternalFormatInt, GL_UNSIGNED_BYTE, NULL);

  //		//Wrap
  //		if (node->FirstChildElement("WARP")) {
  //			const char* WarpOptions =
  // node->FirstChildElement("WARP")->GetText(); 			if
  // (!strcmp(WarpOptions, "REPEAT")) {
  ...
  //			}
  //			else {
  //				cout << "Can't resolve texture wrap type" <<
  // endl; 				return false;
  //			}
  //		}
  //		else {
  ...
  // node->FirstChildElement("FILTER")->GetText(); 			if
  // (!strcmp(FilterOptions, "NEAREST")) {
