...
void CMesh::saveMesh(const CMesh* v_mesh, string v_outName) {
  ...
  fb_binary.open(v_outName, std::ios::out | std::ios::binary);
  std::ostream outstream_binary(&fb_binary);
  if (outstream_binary.fail())
    throw std::runtime_error("failed to open " + v_outName);

  std::vector<float3> positions(v_mesh->vertices.size());
  std::vector<float3> normals(v_mesh->vertices.size());
  std::vector<float3> colors(v_mesh->vertices.size());
  std::vector<uint3> faces(v_mesh->indices.size() / 3);

  const std::vector<Vertex>& vertices = v_mesh->vertices;
  const std::vector<unsigned int>& indices = v_mesh->indices;

  for (size_t i = 0; i < positions.size(); ++i) {
    positions[i].x = vertices[i].Position.x;
    positions[i].y = vertices[i].Position.y;
    positions[i].z = vertices[i].Position.z;

    normals[i].x = vertices[i].Normal.x;
    normals[i].y = vertices[i].Normal.y;
    normals[i].z = vertices[i].Normal.z;

    colors[i].x = vertices[i].Color.x;
    colors[i].y = vertices[i].Color.y;
    colors[i].z = vertices[i].Color.z;
  }

  PlyFile outPLYFile;

  outPLYFile.add_properties_to_element(
      "vertex", {"x", "y", "z"}, Type::FLOAT32, positions.size(),
      reinterpret_cast<uint8_t*>(positions.data()), Type::INVALID, 0);

  outPLYFile.add_properties_to_element(
      "vertex", {"nx", "ny", "nz"}, Type::FLOAT32, normals.size(),
      reinterpret_cast<uint8_t*>(normals.data()), Type::INVALID, 0);

    outPLYFile.add_properties_to_element(
      "vertex", {"r", "g", "b"}, Type::FLOAT32, colors.size(),
      reinterpret_cast<uint8_t*>(colors.data()), Type::INVALID, 0);


  outPLYFile.get_comments().push_back("generated by tinyply 2.2");
