...
CTriMesh::CTriMesh() {
  ...
  for (int i = 0; i < 6; ++i) this->indices.push_back(WindowIndex[i]);

  setupMesh();
}

CTriMesh::CTriMesh(std::vector<Vertex> &vertices,
                   std::vector<unsigned int> &indices, MeshMaterial &material,
...
void CTriMesh::setMesh(std::vector<Vertex> vertices,
  ...
  this->bounds =
      Bounds3f(glm::vec3(minx, miny, minz), glm::vec3(maxx, maxy, maxz));
}
...
void CTriMesh::setupMesh() {
  ...
  // A great thing about struct is that their memory layout is sequential for
...
void CTriMesh::setupMeshNoIndices() {
  // create buffers/arrays
  glGenVertexArrays(1, &VAO);
  glGenBuffers(1, &VBO);
